...
def get_contract_info(product_name):
    ...
    # split, convert to float and convert to pandas series
    ans_dict['prov_per_score'] = pd.Series(
            [float(x) for x in ans[0].split(',')],
            index = get_scores())
    ans_dict['pay_per_score'] = pd.Series(
            [float(x) for x in ans[1].split(',')],
            index = get_scores())
    ans_dict['prepay_per_score'] = pd.Series(
            [float(x) for x in ans[2].split(',')],
            index = get_scores())
    ans_dict['writeoff_per_score'] = pd.Series(
            [float(x) for x in ans[3].split(',')],
            index = get_scores())
